...
function WorkingHoursModal({ barber, onClose }: { barber: Barber; onClose: () => void }) {
  const [hours, setHours] = useState<DayWorkingHourPayload[] | null>(null);
...
export default function AdminBarbeiros() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<BarberForm>();
