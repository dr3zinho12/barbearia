...
import { businessHoursService, DayHourPayload } from '../../services/businessHours.service';
import { Barber, BlockedSchedule } from '../../types';
...
export default function AdminHorarios() {
  ...
  const [hours, setHours] = useState<DayHourPayload[] | null>(null);
  const [isSavingHours, setIsSavingHours] = useState(false);

  const [blocked, setBlocked] = useState<BlockedSchedule[] | null>(null);
  const [barbers, setBarbers] = useState<Barber[]>([]);
  const [isSubmittingBlock, setIsSubmittingBlock] = useState(false);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<BlockedForm>();
  ...
  function updateDay(dayOfWeek: number, changes: Partial<DayHourPayload>) {
  ...
  async function onSubmitBlock(data: BlockedForm) {
