  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Gerencie sua conta de administrador e a identidade do sistema.</p>
      </div>

      <div className="card p-6">
        <h2 className="text-lg font-semibold text-white">Identidade da barbearia</h2>
        <p className="mt-2 text-sm text-slate-400">
          Nome atual: <span className="font-medium text-white">{brand.name}</span>
        </p>
        <p className="mt-2 text-xs text-slate-500">
          Para renomear o sistema, altere <code className="rounded bg-white/5 px-1.5 py-0.5">frontend/src/config/brand.ts</code>{' '}
          e <code className="rounded bg-white/5 px-1.5 py-0.5">backend/src/config/business.ts</code>.
        </p>
      </div>
      ...
      <div className="card p-6">
        ...
        <form className="mt-5 space-y-4" onSubmit={profileForm.handleSubmit(onSubmitProfile)} noValidate>
          <div>
            ...
            <input id="name" className="input" {...profileForm.register('name', { required: 'Informe seu nome' })} />
            {profileForm.formState.errors.name && <p className="field-error">{profileForm.formState.errors.name.message}</p>}
          </div>
          <div>
            <label className="label" htmlFor="email-readonly">E-mail</label>
      ...
      <div className="card p-6">
        ...
        <form className="mt-5 space-y-4" onSubmit={passwordForm.handleSubmit(onSubmitPassword)} noValidate>
          <div>
            ...
            <input id="currentPassword" type="password" className="input" {...passwordForm.register('currentPassword', { required: 'Informe sua senha atual' })} />
            {passwordForm.formState.errors.currentPassword && <p className="field-error">{passwordForm.formState.errors.currentPassword.message}</p>}
          </div>
          <div>
            ...
            />
            {passwordForm.formState.errors.newPassword && <p className="field-error">{passwordForm.formState.errors.newPassword.message}</p>}
          </div>
          <div>
            ...
            />
            {passwordForm.formState.errors.confirmPassword && <p className="field-error">{passwordForm.formState.errors.confirmPassword.message}</p>}
          </div>
